...
def _get_cliente_service(db: Session) -> CrudService[ClienteModel]:
    return CrudService(
        ...
        search_fields=[ClienteModel.codigo, ClienteModel.razao_social, ClienteModel.nome_fantasia],
    )
...
def _get_centro_service(db: Session) -> CrudService[CentroTrabalhoModel]:
    return CrudService(
        ...
        search_fields=[CentroTrabalhoModel.codigo, CentroTrabalhoModel.nome, CentroTrabalhoModel.tipo_maquina],
    )
...
def _get_produto_service(db: Session) -> CrudService[ProdutoFinalModel]:
    return CrudService(
        ...
        search_fields=[ProdutoFinalModel.codigo, ProdutoFinalModel.descricao, ProdutoFinalModel.revisao_atual],
    )
...
def admin_check(_: Annotated[UserRole, Depends(require_roles(UserRole.ADMIN, UserRole.PCP))]) -> dict[str, str]:
...
@router.delete("/clientes/{cliente_id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response)
...
@router.delete("/insumos/{insumo_id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response)
...
@router.post("/centros-trabalho", response_model=CentroTrabalhoResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/centros-trabalho/{centro_id}", response_model=CentroTrabalhoResponse)
...
@router.delete("/centros-trabalho/{centro_id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response)
...
@router.post("/produtos-finais", response_model=ProdutoFinalResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/produtos-finais/{produto_id}", response_model=ProdutoFinalResponse)
...
@router.delete("/produtos-finais/{produto_id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response)
